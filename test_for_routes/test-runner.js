...
function displayTemplate(niceDate, all) {
  return `<!DOCTYPE html>
    <html>
      ...
      <body>
        <script>
          render('${niceDate.split('.').join(':')}', ${JSON.stringify(
    all,
    '',
    '  '
  )})
...
function render(niceDate, all) {
  ...
  for (let t of all) {
    let failedOn,
      badError = t.error && !t.error.includes('AssertionError')
    for (let i in t) {
      if (i === 'error') {
        continue
      }
      if (i === 'name') {
        ...
        let runInfo =
          runs.length < 2
            ? ''
            : ', run ' + (runs.indexOf(t) + 1) + '/' + runs.length
      ...
      if (i === 'tests') {
        ...
        t[i] = t[i]
          .map((x, i) => {
            ...
            x[0] === '*' && (failedOn = i + 1)
            let r =
              `<pre class="a-test ${_class}">${x.slice(x[0] === '*')}` +
              `${x[0] === '*' ? '<hr>' + t.error : ''}</pre>`
            x[0] === '*' && (_class = 'undone')
            return r
          })
      ...
      html += `
        ...
        ${
          i === 'tests'
            ...
            : `<pre class="${i} ${t[i]}"
        >`
        }${t[i].substr ? t[i] : JSON.stringify(t[i], '', '  ')}${
        i === 'tests' ? '' : `</pre>`
      }
      `
    }
